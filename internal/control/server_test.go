...
func (f *fakeBroker) Execute(_ context.Context, request protocol.OperationRequest) (protocol.OperationResponse, error) {
	...
	return protocol.OperationResponse{
		...
		AuditID:    "audit-1",
		Result:     map[string]any{"hostname": "node-1", "unit": request.Parameters["unit"]},
		Error:      f.executeProblem,
	}, nil
}
...
func TestInvalidPlanReturnsBadRequest(t *testing.T) {
	...
	response, err := http.Post(
		server.URL+"/v1/operations/plan",
		"application/json",
		bytes.NewBufferString(`{"action":"shell.exec"}`),
	)
	if err != nil {
		t.Fatal(err)
	}
...
func TestRejectedExecutionReturnsBadRequest(t *testing.T) {
	broker := &fakeBroker{
		...
		executeProblem: &protocol.Problem{Code: "unknown_action", Message: "action is not allowlisted"},
	}
	server := httptest.NewServer(newTestServer(broker, ""))
	defer server.Close()

	response, err := http.Post(
		server.URL+"/v1/operations/execute",
		"application/json",
		bytes.NewBufferString(`{"action":"shell.exec"}`),
	)
	if err != nil {
