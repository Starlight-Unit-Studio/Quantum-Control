...
func (f *fakeBroker) Plan(_ context.Context, request protocol.OperationRequest) (protocol.OperationPlan, error) {
	f.last = request
	if f.planErr != nil {
		return protocol.OperationPlan{}, f.planErr
	}
	valid := true
	if f.planValid != nil {
		valid = *f.planValid
...
func (f *fakeBroker) Execute(_ context.Context, request protocol.OperationRequest) (protocol.OperationResponse, error) {
	f.last = request
	if f.executeErr != nil {
		return protocol.OperationResponse{}, f.executeErr
	}
...
func TestReadyReportsBrokerFailureWithoutLeakingInternalError(t *testing.T) {
	...
	server := httptest.NewServer(newTestServer(broker, ""))
	defer server.Close()
	...
	if strings.Contains(string(body), "/run/private.sock") {
		t.Fatalf("internal broker error leaked: %s", body)
	}
...
func TestSystemStatusUsesTypedBrokerAction(t *testing.T) {
	...
	if broker.last.RequestID == "" {
		t.Fatal("system status did not propagate the HTTP request ID")
	}
}
func TestServiceStatusUsesPathAsTypedParameter(t *testing.T) {
	...
	server := httptest.NewServer(newTestServer(broker, ""))
	defer server.Close()
	response, err := http.Get(server.URL + "/v1/services/quantum-runtime.service")
	if err != nil {
		t.Fatal(err)
	}
	response.Body.Close()
	if response.StatusCode != http.StatusOK || broker.last.Parameters["unit"] != "quantum-runtime.service" {
		t.Fatalf("unexpected status/request: %d %#v", response.StatusCode, broker.last)
	}
}
func TestControlAuthenticationRequiresBearerScheme(t *testing.T) {
	server := httptest.NewServer(newTestServer(&fakeBroker{}, testAPIToken))
	defer server.Close()
	request, _ := http.NewRequest(http.MethodGet, server.URL+"/v1/control/info", nil)
	request.Header.Set("Authorization", testAPIToken)
	...
	if response.StatusCode != http.StatusUnauthorized {
		t.Fatalf("raw token without Bearer scheme was accepted: %d", response.StatusCode)
	}
}
func TestControlAuthenticationAcceptsValidBearerToken(t *testing.T) {
	...
	response.Body.Close()
	if response.StatusCode != http.StatusOK {
		t.Fatalf("valid bearer token was rejected: %d", response.StatusCode)
	}
}
func TestInvalidPlanReturnsBadRequest(t *testing.T) {
	valid := false
	broker := &fakeBroker{planValid: &valid}
	server := httptest.NewServer(newTestServer(broker, ""))
	defer server.Close()
	response, err := http.Post(server.URL+"/v1/operations/plan", "application/json", bytes.NewBufferString(`{"action":"shell.exec"}`))
	if err != nil {
		t.Fatal(err)
	}
	defer response.Body.Close()
	if response.StatusCode != http.StatusBadRequest {
		t.Fatalf("invalid plan returned %d", response.StatusCode)
	}
}
func TestRejectedExecutionReturnsBadRequest(t *testing.T) {
	...
	server := httptest.NewServer(newTestServer(broker, ""))
	defer server.Close()
	response, err := http.Post(server.URL+"/v1/operations/execute", "application/json", bytes.NewBufferString(`{"action":"shell.exec"}`))
	if err != nil {
		t.Fatal(err)
	}
	defer response.Body.Close()
	if response.StatusCode != http.StatusBadRequest {
		t.Fatalf("rejected execution returned %d", response.StatusCode)
	}
}
func TestOversizedOperationBodyReturnsRequestTooLarge(t *testing.T) {
	cfg := testConfig("")
	cfg.RequestBodyLimit = 1024
	logger := slog.New(slog.NewTextHandler(io.Discard, nil))
	server := httptest.NewServer(NewServer(&fakeBroker{}, cfg, logger).Handler())
	defer server.Close()
	body := `{"action":"system.snapshot","confirmation":"` + strings.Repeat("x", 2048) + `"}`
	response, err := http.Post(server.URL+"/v1/operations/plan", "application/json", strings.NewReader(body))
	if err != nil {
		t.Fatal(err)
	}
	response.Body.Close()
...
func newTestServer(client *fakeBroker, token string) http.Handler {
	logger := slog.New(slog.NewTextHandler(io.Discard, nil))
	return NewServer(client, testConfig(token), logger).Handler()
}
func testConfig(token string) config.Control {
	return config.Control{Listen: "127.0.0.1:17440", APIToken: token, AuditPath: "/tmp/quantum-control-audit-test.jsonl", PlanTTL: 5 * time.Minute, BrokerSocket: "/tmp/qcored.sock", BrokerToken: testAPIToken, RequestBodyLimit: 1 << 20, HeaderTimeout: 10 * time.Second, IdleTimeout: 90 * time.Second, BrokerTimeout: 15 * time.Second}
}
